...
@Component({
  selector: 'app-patient-consultation-dashboard',
  templateUrl: './patient-consultation-dashboard.component.html',
  styleUrls: ['./patient-consultation-dashboard.component.scss']
})
export class PatientConsultationDashboardComponent implements OnInit, OnDestroy {
  currentUser: any;
  
  // Consultation lists
  upcomingConsultations: VideoConsultation[] = [];
  activeConsultations: VideoConsultation[] = [];
  pastConsultations: VideoConsultation[] = [];
  ...
  private categorizeConsultations(consultations: VideoConsultation[]): void {
    ...
    this.upcomingConsultations = consultations.filter(c => 
      ['SCHEDULED', 'WAITING_FOR_DOCTOR'].includes(c.status) &&
      new Date(c.scheduledStartTime) > now
    );
    
    this.activeConsultations = consultations.filter(c => 
      ['IN_PROGRESS', 'WAITING_FOR_PATIENT'].includes(c.status)
    );
    
    this.pastConsultations = consultations.filter(c => 
  ...
  getTabCount(tab: string): number {
    switch (tab) {
      ...
      case 'past': return this.pastConsultations.length;
      default: return 0;
    }
  }

  getDoctorName(consultation: VideoConsultation): string {
    return `Dr. ${consultation.doctor.firstName} ${consultation.doctor.lastName}`;
  }

  getDoctorSpecialization(consultation: VideoConsultation): string {
    return consultation.doctor.specialization || 'General Practice';
  }
}
